...
contract EscrowLink {
    ...
    function getEscrowDetails(uint16 _escrow_id) public OnlyPayee(_escrow_id) returns (address, address, uint256, bool, string memory, uint16) {
        Escrow memory escrow = escrows[_escrow_id];
        emit PayeeHasJoined(escrow.escrow_id);
        return (escrow.payer, escrow.payee, escrow.amount, escrow.isPaid, escrow.terms, escrow.escrow_id);
    }
